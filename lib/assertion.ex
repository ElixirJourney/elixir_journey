  ...
  defmacro __before_compile__(_env) do
    quote do
      def run(exercise_file) do
        IO.puts("running all my test cases #{inspect @tests}")
...
defmodule ElixirJourney.Test do
  ...
  def run(tests, module, exercise_file) do
    Enum.each(tests, fn({test_fn, description}) ->
      case apply(module, test_fn, []) do
        :ok -> IO.write("Test case #{description} pass\n")
        {:fail, reason} -> IO.puts """
        ============================
        Failure: #{description}
        ============================
        #{reason}
        """
        response -> IO.write("Teest case #{response} for #{description}")
      end

    end)
  end
  def assert(left, right, :function) do
    :function
  end
